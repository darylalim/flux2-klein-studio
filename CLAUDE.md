...
4. **UI** — Behind `if __name__ == "__main__"` (`layout="wide"`, modeled on the FLUX.2 [klein] Gradio space): `st.title` header (`FLUX.2 Klein Pipeline`) plus a one-line `st.markdown` description with `[model]`/`[blog]` links. A two-column studio layout via `st.columns([2, 3])` — **left** holds all controls, **right** holds the output (two reserved `st.empty()` slots: `progress_slot` for the per-step `st.progress` bar and `result_slot` for the image; `result_slot` renders an HTML placeholder canvas until a run completes). Left column, top to bottom: a prompt `st.text_input` (placeholder `Enter your prompt`, key `prompt_input`) + a `Run` button inline via `st.columns([4, 1])`; an `Input image(s) (optional)` expander wrapping the file uploader (`accept_multiple_files=True`, label collapsed) — there is **no Task toggle**, uploading any image routes `infer()` to `Flux2KleinEdit` automatically; a `Mode` `st.radio` (`horizontal=True`, key `mode_radio`, options `Distilled (4 steps)`/`Base (50 steps)` mapped back to internal `Fast`/`Quality` via `LABEL_TO_MODE`); an `Advanced Settings` expander (collapsed by default) holding a `Prompt Upsampling` checkbox (key `auto_enhance_checkbox`) + `st.caption`, a `Randomize seed` checkbox + a `Seed` `st.number_input` (disabled while Randomize is on), Width/Height sliders, and Number of inference steps/Guidance scale sliders; an `Examples` section of clickable `EXAMPLE_PROMPTS` buttons (truncated labels via `_truncate`, full prompt on hover) that fill the prompt box via `_set_example_prompt` (sets `st.session_state.prompt_input`, clears any example images); and a second `Examples` section (editing examples) from `EDIT_EXAMPLES` (`(prompt, [image paths])` tuples, verbatim from the Gradio space) — each row is a prompt button + thumbnail `st.image`, and clicking calls `_load_edit_example` to set the prompt and store the bundled image paths in `st.session_state.example_images`. Because `st.file_uploader` cannot be set programmatically, `image_list` falls back to `example_images` (opened with `Image.open`) when no manual upload is present, so editing examples route through `Flux2KleinEdit` exactly like uploads; a manual upload clears `example_images`, and a `Clear example images` button (in the uploader expander, which auto-expands once when an example is loaded via a one-shot `expand_input_once` flag) removes them. Example images are bundled under `examples/` (`woman1.webp`, `cat_window.webp`, `bird.webp`), resolved via `_EXAMPLES_DIR = Path(__file__).resolve().parent / "examples"`. The Run button is always enabled (editing is implicit). Width/height sliders auto-update to match the first input image's aspect ratio (uploaded or example); guidance scale and steps update when the mode changes.
