...
| Don't drop `wrap=True` from the example buttons | Since Streamlit 1.63 a button placed directly in a column defaults to one ellipsized line (~30 chars here), hiding most of each label and drifting from the README hero; `AppTest` cannot see layout | `test_example_buttons_wrap_their_labels` |
...
4. **UI** — behind `if __name__ == "__main__"` (`layout="wide"`, modeled on the FLUX.2 [klein] Gradio space): a lone `st.title` header and a two-column studio via `st.columns([2, 3])`, left for all controls, right for the output. Read the file for the widget inventory; these are the invariants it won't tell you:
   ...
   - **The theme is Streamlit's own.** `.streamlit/config.toml` declares no `[theme]` section at all, so the stock light and dark themes apply and the in-app appearance switcher picks between them. That absence is the setting: adding back *any* `[theme]` key makes it a custom theme, and a custom theme with **neither** `[theme.light]` nor `[theme.dark]` populated collapses to one `Custom Theme` with no light/dark switcher — the frontend's variant branch (`xn` in 1.64.0's `static/js/utils.*.js`; the minified name moves every release, it was `Lu` in 1.61.1) tests `light || dark`, so one populated variant is enough to keep it, while a lone `font` populates neither and costs it. `TestThemeConfig` locks the absence; the palette's contrast is Streamlit's contract, not this repo's. **Accepted with a known cost:** Streamlit's stock `#FF4B4B` primary renders white button labels at 3.30:1, under WCAG AA's 4.5:1 for normal text — measured on the Run button in `docs/screenshot-dark.png`, where the retired custom palette passed at 5.19:1 light / 4.62:1 dark. Taken deliberately as upstream's call, which is why no contrast test survives; a `primaryColor`-only `[theme.light]`/`[theme.dark]` pair would restore AA without giving up the rest of the stock theme. No custom CSS either.
...
Cut by bumping `version` in `pyproject.toml` and pushing to `main` — nothing else. `uv version --bump <major|minor|patch>` does the bump *and* re-locks in one step (uv 0.12.18 re-locks by default; `--frozen` is the opt-out), which is the whole reason to prefer it over hand-editing: a forgotten `uv lock` is exactly the mistake CI's `--locked` exists to catch. Run `uv run pytest -m smoke` **before** bumping.
...
- **`docs/screenshot-dark.png` is the README hero** — one shot, dark only; there is no light counterpart. `tests/test_readme.py` locks that the README's local images exist and are git-tracked, never that they match the UI, so any change to the controls column means re-shooting. Last re-shot 2026-08-20; it matched the UI as of `6916c76` (Streamlit default dark, no Mode toggle), and a 2026-09-23 render on Streamlit 1.64.0 still matched once the example buttons took `wrap=True` — nothing re-checks that afterwards. The recipe, because two obvious routes both fail: serve the app with `--theme.base dark --client.toolbarMode minimal` (forces dark deterministically and hides the Deploy toolbar). That flag *is* a custom theme, and the page renders down the custom-theme path — but `base` alone merges onto the stock Dark palette, so the pixels are the stock theme's; verified on the asset, background `#0E1117` and primary `#FF4B4B`. Then, then drive headless Chrome over CDP on `--remote-debugging-port`, `Emulation.setDeviceMetricsOverride` at 1440×880 / `deviceScaleFactor: 2`, polling `document.body.innerText` until the app renders before `Page.captureScreenshot`. Chrome's one-shot `--screenshot` flag fires at the load event and catches Streamlit's skeleton placeholder instead, and macOS `screencapture` can only reach 1× on a non-Retina display — this machine has none, and the asset is 2×. Finally crop the bottom to match the top margin: the left controls column runs longer than the right output frame, so a raw 1440×880 capture ships ~22% dead canvas, and the committed 2880×1617 is that capture trimmed to a balanced 249px top / 250px bottom.
