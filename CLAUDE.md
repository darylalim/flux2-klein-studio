...
| `.streamlit/config.toml` | native theming |
...
4. **UI** — behind `if __name__ == "__main__"` (`layout="wide"`, modeled on the FLUX.2 [klein] Gradio space): a lone `st.title` header and a two-column studio via `st.columns([2, 3])`, left for all controls, right for the output. Read the file for the widget inventory; these are the invariants it won't tell you:
   ...
   - **Theming is native**, in `.streamlit/config.toml`: matched light/dark palettes, Inter, `baseRadius`, plus a dark-mode `linkColor` override because dark links otherwise inherit `primaryColor` below WCAG AA. `TestThemeConfig` asserts the link/text/button contrasts meet AA (`ratio ≥ 4.5:1`) and that the dark `linkColor` differs from `primaryColor` — it locks the thresholds, not the measured ratios. No custom CSS, so the in-app appearance switcher stays available.
