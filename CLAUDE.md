...
Application code lives in `streamlit_app.py`, structured in four sections (with the test suite in `tests/`, bundled example images in `examples/`, repo-shared Claude Code hooks in `.claude/`, and the CI workflow in `.github/workflows/`; `README.md` mirrors this file's Overview/Setup/CI/Hooks content for a public audience — keep the two in sync):
